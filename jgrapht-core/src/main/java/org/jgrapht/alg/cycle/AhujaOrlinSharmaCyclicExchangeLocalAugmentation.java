...
/**
 * Implementation of an algorithm for the local augmentation problem for the
 * cyclic exchange
 * neighborhood, i.e. it finds subset-disjoint negative cycles in a graph, based
 * on Ravindra K.
 * Ahuja, James B. Orlin, Dushyant Sharma, A composite very large-scale
 * neighborhood structure for
 * the capacitated minimum spanning tree problem, Operations Research Letters,
 * Volume 31, Issue 3,
 ...
 *
 * A subset-disjoint cycle is a cycle such that no two vertices in the cycle are
 * in the same subset
 * of a given partition of the whole vertex set.
 *
 * This algorithm returns the first or the best found negative subset-disjoint
 * cycle. In the case of
 * the first found cycle, the cycle has minimum number of vertices. It may
 * enumerate all paths up to
 * the length given by the parameter {@code lengthBound}, i.e the algorithm runs
 * in exponential
 * time.
 *
 * This algorithm is used to detect valid cyclic exchanges in a cyclic exchange
...
public class AhujaOrlinSharmaCyclicExchangeLocalAugmentation<V, E> {
    ...
    /**
     * the map that maps each vertex to a subset (identified by labels) of the
     * partition
     */
     ...
     *
     * @param graph           the directed graph on which to find the negative
     *                        subset disjoint cycle. The
     *                        vertices of the graph are labeled according to
     *                        labelMap.
     * @param lengthBound     the (inclusive) upper bound for the length of cycles
     *                        to detect
     * @param labelMap        the labelMap of the vertices encoding the subsets of
     *                        vertices
     * @param bestImprovement contains whether the best or the first improvement is
     *                        returned: best
    ...
    /**
     * Calculates a valid subset-disjoint negative cycle. If there is no such cycle,
     * it returns an
    ...
    public GraphWalk<V, E> getLocalAugmentationCycle() {
        ...
        /*
         * Store the path in map with key PathSetKey<V, V, Set<Integer>>, since only
         * paths with the
         * same head, same tail, and the subset of labels may be in domination relation.
         * Thus the
    ...
    /**
     * Checks whether {@code path} dominates the current minimal cost path with the
     * same head,
     * tail and label set in the set of all paths of length k + 1. Thus, dominated
     * paths are
     * eliminated. This is important out of efficiency reasons, otherwise many
     * unnecessary paths may
     ...
     *
     * @return whether {@code path} dominates the current minimal cost path with the
     *         same head,
     *         tail and label set.
     */
    private boolean checkDominatedPathsOfLengthKplus1(
        ...
        LabeledPath<V> pathToCheck = pathsLengthKplus1
                .get(new PathSetKey<>(path.getHead(), path.getTail(), path.labels));
        if (pathToCheck != null) {
    ...
    /**
     * Checks whether {@code path} is dominated by some path in the previously
     * calculated set
     * of paths of length k. This is important out of efficiency reasons, otherwise
     * many unnecessary
     ...
     *
     * @return whether {@code path} is dominated by some path in
     *         {@code pathsLengthK}
     */
    ...
    /**
     * Adds a path and removes the path, which has the same tail, head and label
     * set, to the data
     * structure {@code paths}, which contains all paths indexed by their head, tail
     * and label
     * set.
     *
     * @param paths the map of paths, which are indexed by head, tail and label set,
     *              to add the path
     *              to
